                             ...
                             void ***pMappedList)
{
    ...
    //*uniformBuffersMemory = malloc(sizeof(VkDeviceMemory) *
    ...
    *pMappedList                = uniformBuffersMapped;
    ...
    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        VK_CHECK(vkMapMemory(
          device, buffersMemory[i], 0, size, 0, &uniformBuffersMapped[i]));
    }
                             ...
                             VkExtent2D swapchainExtent)
{
    ...
    glm_lookat(
      (vec3) {2.0f, 2.0f, 2.0f}, (vec3) {0, 0, 0}, (vec3) {0, 0, 1}, ubo.view);

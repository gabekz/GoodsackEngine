...
Skybox *skybox_create(Texture *cubemap) {
    ...
    IBO *ibo = ibo_create(PRIM_ARR_I_CUBE, PRIM_SIZ_I_CUBE * sizeof(unsigned int));
    ibo_bind(ibo);
    free(vbo);

    ShaderProgram *shader = shader_create_program("../res/shaders/skybox.shader");
    ret->shader = shader;
...
void skybox_draw(Skybox *self) {
    ...
    glBindTexture(GL_TEXTURE_2D, self->cubemap->id);

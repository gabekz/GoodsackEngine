...
static gsk_Model *
_create_model_from_batches(ArrayList *list_batches)
{
    if (list_batches->is_list_empty) { return NULL; }

    ArrayList list_mesh_ptrs = LIST_INIT(sizeof(gsk_Mesh **), 1);
    ...
    gsk_Model *ret_model   = malloc(sizeof(gsk_Model));
    ret_model->meshes      = list_mesh_ptrs.data.buffer;
    ret_model->meshesCount = LIST_COUNT(&list_mesh_ptrs) + 1;
    ret_model->modelPath   = "NONE";
    ret_model->fileType    = QMAP;
...
// TODO: move
    ...
    for (int i = 0; i < p_container->list_entities.list_next; i++)
    {
        ...
        if (ent->list_brushes.is_list_empty == FALSE && no_render == FALSE)
        {
            /* vertex-batch list */
            ent->list_batches = LIST_INIT(sizeof(struct _BatchInfo), 1);
            ...
            LIST_PUSH(&ent->list_batches, &batch_err);
        }
        ...
        for (int j = 0; j < ent->list_brushes.list_next; j++)
        {
            ...
            glm_vec3_sub(maxBounds, brush->world_pos, brush->brush_bounds[1]);
                ...
                if (poly->p_texture == NULL)
                {
                    ...
                    struct _BatchInfo *p_batch =
                      LIST_GET(&ent->list_batches, 0);
                ...
                for (int m = 0; m < ent->list_batches.list_next; m++)
                {
                    // break out immediately to start making a new material
                    if (ent->list_batches.is_list_empty) { break; }

                    struct _BatchInfo *p_batch =
                      LIST_GET(&ent->list_batches, m);
                ...
                if (is_new_material == TRUE)
                {
                    ...
                    LIST_PUSH(&ent->list_batches, &batch);
                }
            }
        }

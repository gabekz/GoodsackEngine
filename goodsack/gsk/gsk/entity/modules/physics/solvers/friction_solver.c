...
impulse_solver_with_rotation_friction(_SolverData solver_data)
{
    ...
    // calculate r-values + relative velocity
    {
        ...
        __calc_relative_velocity(solver_data, 
                ra_perp, rb_perp, body_a.linear_velocity, body_a.angular_velocity, 
                body_b.linear_velocity, body_b.angular_velocity, relative_velocity);
    }
    ...
    {
        __calc_relative_velocity(solver_data, 
                ra_perp, rb_perp, rigidbody_a->linear_velocity, rigidbody_a->angular_velocity, 
                body_b.linear_velocity, body_b.angular_velocity, relative_velocity);
    }
    ...
    vec3 friction_impulse, friction_torque = GLM_VEC3_ZERO_INIT;
    {
        ...
        //  scale torque by inverse inertia
        glm_vec3_scale(friction_torque, body_a.inverse_inertia, friction_torque);
    ...
    {
        glm_vec3_add(
          rigidbody_a->linear_velocity, friction_impulse, rigidbody_a->linear_velocity);

        glm_vec3_add(
          rigidbody_a->angular_velocity, friction_torque, rigidbody_a->angular_velocity);
    }
